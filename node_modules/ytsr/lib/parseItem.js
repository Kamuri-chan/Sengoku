...
const catchAndLog_parseItem = item => {
  try {
    ...
    let ePrint = `failed to parse Type: ${e.message}`;
...
const parseVideo = obj => {
  ...
  let authorUrl = null;
  if (author) authorUrl = author.navigationEndpoint.browseEndpoint.canonicalBaseUrl ||
...
const parseMovie = obj => {
  ...
  return {
    ...
    author: {
      name: owner.text,
      channelId: owner.navigationEndpoint.browseEndpoint.browseId,
      link: URL.resolve(BASE_VIDEO_URL, owner.navigationEndpoint.browseEndpoint.canonicalBaseUrl || targetUrl),
